...
describe('Resume', () => {
  ...
  test('education あり', () => {
    ...
    wrapper = factory(state, employmentState(), skillsState())
    expect(wrapper.find('.education').html()).toContain('東京大学')
  })
  ...
  test('languages あり', () => {
    ...
    state.languages[0] = {
      name: 'JavaScript',
      description: 'Nuxt.jsでアプリケーションを作成'
    }
  ...
  test('links あり', () => {
    ...
    wrapper = factory(detailsState(), employmentState(), state)
    expect(wrapper.find('.link').html()).toContain('https://www.github.com')
  })
  ...
  test('tools あり', () => {
    ...
    wrapper = factory(detailsState(), employmentState(), state)
    expect(wrapper.find('.tool').html()).toContain('AWS Lambda')
  })
